...
public class DriverRepository implements CrudRepository<Driver, Long> {
    ...
    private final static String SQL_UPDATE_BY_ID = "update driver set (id, first_name, last_name, age) VALUES (?, ?, ?, ?) where id = ?;";
    private final static String SQL_DELETE_BY_ID = "delete from (select * from driver) where id = ?;";
    ...
    public void update(Long id, Driver item) {
        try {
            ...
            if (lineChanged == 0) {
                System.out.println("NO UPDATE! lineChanged: " + lineChanged);
                throw new SQLException();
            } else System.out.println("Update, lineChanged: " + lineChanged);
    ...
    public void delete(Long id) throws SQLException {
        try {
            ...
            int lineChanged = statement.executeUpdate();
            if (lineChanged == 0) {
                System.out.println("NO DELETE! lineChanged: " + lineChanged);
                throw new SQLException();
            } else System.out.println("Deleted, lineChanged: " + lineChanged);
        } catch (SQLException e) {
            e.getLocalizedMessage();
        }
    }
